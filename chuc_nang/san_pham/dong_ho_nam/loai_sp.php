...
$sql = "select * from san_pham join ct_san_pham on san_pham.masp=ct_san_pham.masp join danh_muc_sp on ct_san_pham.doi_tuong_sd=danh_muc_sp.id_danh_muc where ct_san_pham.doi_tuong_sd like 1 and loaisp like '%$loai_sp%'";
...
<script>
	$(document).ready(function() {
		...
		$("#male-page").addClass("active");
		document.title = "Đồng Hồ <?php echo ucfirst($loai_sp) ?> Nam";
	});
</script>
<div class="container px-0 mt-2"><img style="width:100%" src="images/dong-ho-nam-banner.webp" alt=""></div>
<div class="main_container" onload="title()">
	<div class="dst_hphukien">
		<h3 class="col-md-5 float-left mt-2 px-0">Đồng Hồ <?php echo ucfirst($loai_sp) ?> Nam</h3>
		<?php include('../watch/chuc_nang/san_pham/sort.php'); ?>
		<div class="float-right mt-2 pt-1">
			<a href="?menu=male_loaisp&id=casio" class="btn btn-sm  btn-outline-primary">Casio</a>
			<a href="?menu=male_loaisp&id=doxa" class="btn btn-sm  btn-outline-primary">Doxa</a>
			<a href="?menu=male_loaisp&id=skmei" class="btn btn-sm  btn-outline-primary">Skmei</a>
			<a href="?menu=male_loaisp&id=tissot" class="btn btn-sm  btn-outline-primary">Tissot</a>
		</div>
	</div>
	<?php if ($total==0) { ?>
	...
	<div class="product-list">
		...
		<?php
		$sl = "select * from san_pham join ct_san_pham on san_pham.masp=ct_san_pham.masp join danh_muc_sp on ct_san_pham.doi_tuong_sd=danh_muc_sp.id_danh_muc where ct_san_pham.doi_tuong_sd like 1 and loaisp like '%$loai_sp%' order by " . $_SESSION['sort'] . " limit " . $start . "," . $limit;
